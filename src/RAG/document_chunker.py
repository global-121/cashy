...
from src.RAG.utils import initialize_logger, num_tokens_from_string
...
class DocumentChunker:
    ...
    def _set_chunker(self):
        ...
        if self.chunking_strategy.lower() == "sentencesplitting":
            ...
            self.logger.info(
                "Using Langchain SpacyTextSplitter for chunking the documents"
            )
    ...
    def _add_chunk_metadata(self, chunked_documents: List[Document]) -> List[Document]:
        ...
        for chunked_doc in chunked_documents:
            ...
            new_metadata[dm.CELL_INDEX] = chunked_doc.metadata[dm.CELL_INDEX]
            ...
            # self.logger.info(f"The length of chunk {document_id}.{nth_chunk} is {chunk_length_in_tokens} tokens and {chunk_length_in_chars} characters.")
